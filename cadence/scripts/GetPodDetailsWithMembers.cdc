...
// Struct to hold complete pod details including all members
access(all) struct PodDetailsWithMembers {
...
// Script to get detailed information about a specific pod including all members
// Arguments:
//   - podID: The ID of the pod to query
access(all) fun main(podID: UInt64): PodDetailsWithMembers? {
    // Borrow a reference to the pod using Pod.borrowPod
    if let pod = Pod.borrowPod(podID: podID) {
        ...
        // Return complete pod details with members and balances
        return PodDetailsWithMembers(
            id: pod.id,
            name: pod.name,
